const fs = require('fs');
const path = 'f:/Roastery Operating System/ros-app';
const files = fs.readFileSync(path + '/graphify-out/.graphify_uncached.txt', 'utf8').split(/\r?\n/).filter(Boolean);

const promptTemplate = `You are a graphify extraction subagent. Read the files listed and extract a knowledge graph fragment.
...
const docs = files.filter(f => !f.match(/\.(png|jpe?g|svg|gif|webp)$/i));
const images = files.filter(f => f.match(/\.(png|jpe?g|svg|gif|webp)$/i));

let chunks = [];
if (docs.length > 0) chunks.push(docs);
images.forEach(img => chunks.push([img]));

chunks.forEach((chunkFiles, i) => {
    let numStr = String(i+1).padStart(2, '0');
    let chunkPath = `${path}/graphify-out/.graphify_chunk_${numStr}.json`;
    let content = promptTemplate
        .replace('{{CHUNK_NUM}}', i+1)
        .replace('{{TOTAL_CHUNKS}}', chunks.length)
        .replace('{{FILE_LIST}}', chunkFiles.join('\n'))
        .replace('{{DEEP_MODE}}', '')
        .replace('{{CHUNK_PATH}}', chunkPath);
    fs.writeFileSync(`${path}/graphify-out/prompt_${numStr}.txt`, content);
});
console.log('Wrote ' + chunks.length + ' prompts');
