...
function LineRow({
  ...
  const rowCls = cn(
    "grid grid-cols-[1fr_auto_auto] items-center gap-x-6 px-5 py-2.5",
  ...
  return (
    <div className={rowCls}>
      ...
      <span className="text-xs text-zinc-400 text-right min-w-[48px]">
        {percentage ?? ""}
      </span>
      <span className={cn("text-sm text-right min-w-[160px]", valueCls)}>
        {showSign && value > 0 ? "+" : ""}
...
function MonthNavigator({ month, year }: { month: number; year: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="min-w-[130px] text-center text-sm font-semibold text-zinc-800">
        {MONTHS[month - 1]} {year}
      </span>
      <Button
        variant="outline"
...
export function PnLReportClient({ report }: PnLReportClientProps) {
  ...
  return (
    <StandardPageLayout
      ...
      actionButton={<MonthNavigator month={month} year={year} />}
    >
      {/* ── KPI Summary Row ── */}
      ...
      <div className="rounded-3xl border border-white/60 bg-white/80 backdrop-blur-md shadow-lg shadow-slate-200/40 overflow-hidden">
        ...
        <div className="border-t border-zinc-100 bg-zinc-50/60 px-5 py-3">
          <div className="flex items-center justify-between text-[11px] text-zinc-400">
            <span>
              Laporan ini digenerate otomatis oleh Roastery OS · Nalweng
            </span>
            ...
            {opexBreakdown.map((item) => {
              ...
              return (
                <div key={item.category} className="flex items-center gap-4 px-5 py-3">
                  <div className="min-w-[160px] text-sm font-medium text-zinc-700">
                  ...
                  <div className="text-right min-w-[140px] tabular-nums text-sm font-semibold text-zinc-800 font-mono">
