...
export function CatalogSection({
  ...
  const FilterPill = ({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) => (
    <button
      ...
      className={`px-4 py-1.5 text-xs font-medium rounded-full transition-all duration-300 ${
        active
          ? "bg-[#6b4423] text-white shadow-[0_2px_8px_rgba(107,68,35,0.15)]"
          : "bg-white text-[#8b7e74] hover:bg-[#f0ebe5] border border-[#e8e0d8]"
      }`}
  ...
  return (
    <section id="catalog" className="w-full bg-[#faf8f5]">
      <div className="max-w-6xl mx-auto px-5 sm:px-8 py-20 md:py-28">
        ...
        >
          ...
          {/* View Mode Toggle */}
          <div className="flex items-center border border-[#e8e0d8] rounded-2xl p-1 bg-white">
            <button
              onClick={() => setViewMode("grid")}
              className={`p-2 rounded-xl transition-all duration-300 ${viewMode === "grid" ? "bg-[#6b4423] text-white shadow-sm" : "text-[#8b7e74] hover:text-[#2c2420]"}`}
              title="Grid View"
            >
            ...
            <button
              ...
              className={`p-2 rounded-xl transition-all duration-300 ${viewMode === "table" ? "bg-[#6b4423] text-white shadow-sm" : "text-[#8b7e74] hover:text-[#2c2420]"}`}
              title="Table View"
            >
        ...
        {customerTier !== "RETAIL" && tierInfo && (
          <motion.div
            ...
            className="flex items-center gap-3 p-4 mb-6 rounded-2xl bg-[#6b4423]/5 border border-[#6b4423]/10"
          >
            <Info size={18} className="text-[#c8956c]" strokeWidth={1.5} />
            <p
              className="text-sm text-[#2c2420]"
              style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
            >
              You&apos;re viewing <span className="font-semibold">{tierInfo.tierLabel}</span> pricing.
              {tierInfo.savingsPercent > 0 && (
                <span className="ml-1 font-semibold text-[#6b4423]">Save up to {tierInfo.savingsPercent}%</span>
              )}
            </p>
          </motion.div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            <Search size={16} strokeWidth={1.5} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#8b7e74]" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search coffee, origin, or category..."
              className="w-full pl-10 pr-10 py-3 text-sm rounded-2xl bg-white border border-[#e8e0d8] text-[#2c2420] placeholder:text-[#8b7e74]/50 focus:outline-none focus:border-[#c8956c] focus:ring-1 focus:ring-[#c8956c]/20 transition-all duration-300"
              style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
            />
            {searchQuery && (
              <button
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 text-sm font-medium rounded-2xl border transition-all duration-300 ${
                showFilters
                  ? "bg-[#6b4423]/5 border-[#6b4423]/20 text-[#6b4423]"
                  : "bg-white border-[#e8e0d8] text-[#8b7e74] hover:border-[#c8956c]/30"
              }`}
              style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
            >
              ...
              Filter
              {activeFilterCount > 0 && (
                <span className="w-5 h-5 flex items-center justify-center text-[10px] font-medium rounded-full bg-[#6b4423] text-white">
                  {activeFilterCount}
                </span>
              )}
            </button>
            ...
            <select
              ...
              className="px-3 py-3 text-sm rounded-2xl bg-white border border-[#e8e0d8] text-[#2c2420] cursor-pointer focus:outline-none focus:border-[#c8956c] transition-all duration-300"
              style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
            >
        ...
        <AnimatePresence>
          {showFilters && (
            ...
            >
              <div className="p-6 rounded-[20px] border border-[#e8e0d8] bg-white space-y-5">
                {categories.length > 2 && (
                  <div>
                    <label
                      className="text-[11px] font-medium uppercase tracking-[0.15em] text-[#8b7e74] mb-2 block"
                      style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
                    >
                      Category
                    </label>
                ...
                {activeFilterCount > 0 && (
                  <button
                    ...
                    className="text-xs font-medium text-[#6b4423] hover:text-[#5a3920] underline underline-offset-2 transition-colors"
                    style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
                  >
                    Reset All Filters
                  </button>
        ...
        {activeFilterCount > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {activeCategory !== "ALL" && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-full bg-[#6b4423]/8 text-[#6b4423]">
                {activeCategory}
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-20 rounded-[20px] border-2 border-dashed border-[#e8e0d8]">
            <Coffee size={48} strokeWidth={1} className="mx-auto mb-4 opacity-20 text-[#8b7e74]" />
            <p
              className="text-[#8b7e74] mb-3"
              style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
            >
            ...
            <button
              onClick={resetFilters}
              className="text-sm font-medium text-[#6b4423] hover:text-[#5a3920] underline underline-offset-2 transition-colors"
              style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
            >
              Reset Filters
            </button>
            ...
            {filteredProducts.map((product, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="group"
                >
                  <div className="relative rounded-[20px] overflow-hidden bg-white border border-[#e8e0d8] transition-all duration-500 group-hover:shadow-[0_16px_40px_rgba(107,68,35,0.08)] group-hover:border-[#c8956c]/30 h-full flex flex-col">
                    {/* Image */}
                    <div className="relative w-full aspect-[4/3] overflow-hidden bg-[#f0ebe5]">
                      {product.imageUrl ? (
                        <img
                      ...
                      {showTierPrice && (
                        <span
                          className="absolute top-3 right-3 text-[10px] uppercase font-medium tracking-[0.1em] px-2.5 py-1 rounded-lg bg-[#c8956c]/90 text-white backdrop-blur-md"
                          style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
                        >
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      <h3
                        className="text-base font-semibold text-[#2c2420] mb-1 line-clamp-1"
                        style={{ fontFamily: "'Playfair Display', 'Source Serif 4', Georgia, serif" }}
                      >
                        {product.name}
                      </h3>
                      <p
                        className="text-xs text-[#8b7e74] line-clamp-2 mb-3 font-normal leading-[1.7]"
                        style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
                      >
                      ...
                      {(product.origin || product.roastLevel) && (
                        <div className="flex items-center gap-2 mb-4 text-[10px] uppercase tracking-[0.1em] text-[#8b7e74] font-medium">
                      ...
                      <div className="mt-auto space-y-3">
                        <div className="flex justify-between items-end bg-[#faf8f5] p-3 rounded-2xl border border-[#e8e0d8]">
                          <div className="flex flex-col">
                            <span
                              className="text-[10px] text-[#8b7e74] uppercase tracking-[0.1em] font-medium"
                              style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
                            >
                              Price
                            </span>
                            {showTierPrice && (
                              <span className="text-[10px] font-semibold text-[#6b4423]">Tier {pricing.tierLabel}</span>
                            )}
                          </div>
                          <div className="text-right">
                            {showTierPrice && (
                              <div className="text-[10px] line-through text-[#8b7e74]/40">
                                {formatPrice(pricing.retailPrice)}
                              </div>
                            )}
                            <span
                              className="text-base font-semibold text-[#6b4423]"
                              style={{ fontFamily: "'Playfair Display', 'Source Serif 4', Georgia, serif" }}
                            >
                        ...
                        {/* MOQ */}
                        {moq > 1 && (
                          <div className="flex items-center gap-1.5 text-[10px] text-[#8b7e74] font-medium">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex items-center border border-[#e8e0d8] rounded-2xl overflow-hidden bg-white">
                            <button
                              onClick={() => handleQtyChange(product.id, -1, moq)}
                              disabled={qty <= moq}
                              className="w-9 h-9 flex items-center justify-center text-sm font-medium hover:bg-[#faf8f5] disabled:opacity-30 transition-colors"
                            >
                              <Minus size={14} strokeWidth={1.5} />
                            </button>
                            <span
                              className="text-sm font-medium w-8 text-center text-[#2c2420]"
                              style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
                            >
                              {qty}
                            </span>
                            <button
                              onClick={() => handleQtyChange(product.id, 1, moq)}
                              className="w-9 h-9 flex items-center justify-center text-sm font-medium hover:bg-[#faf8f5] transition-colors"
                            >
                          ...
                          <motion.button
                            ...
                            disabled={!hasStock}
                            className="flex-1 flex items-center justify-center gap-2 py-2.5 font-medium text-sm rounded-2xl transition-all duration-300 bg-[#6b4423] text-white hover:bg-[#5a3920] disabled:opacity-40 disabled:cursor-not-allowed shadow-[0_2px_8px_rgba(107,68,35,0.15)]"
                            style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
                          >
          ...
          /* ═══ TABLE VIEW ═══ */
          <div className="w-full overflow-x-auto rounded-[20px] border border-[#e8e0d8] bg-white">
            <table className="w-full border-collapse text-left text-sm text-[#2c2420]">
              <thead>
                <tr className="border-b border-[#e8e0d8] bg-[#faf8f5]">
                  {["Product", "Origin", "Category", "Stock", "Price", "Qty", "Action"].map(h => (
                    <th
                      key={h}
                      className="p-4 font-medium text-[11px] uppercase tracking-[0.12em] text-[#8b7e74]"
                      style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
                    >
              ...
              <tbody className="divide-y divide-[#e8e0d8]">
                {filteredProducts.map(product => {
                  ...
                  return (
                    <tr key={product.id} className="hover:bg-[#faf8f5]/60 transition-colors duration-300">
                      <td
                      ...
                      <td
                        className="p-4 text-[#8b7e74] text-xs"
                        style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
                      >
                        {product.origin || "—"}
                      </td>
                      <td className="p-4">
                        <span className="px-2 py-1 text-[10px] font-medium rounded-full bg-[#6b4423]/8 text-[#6b4423] uppercase tracking-[0.1em]">
                          {product.category || "General"}
                        </span>
                      </td>
                      <td className="p-4">
                        <span className={`font-medium text-xs ${hasStock ? "text-[#4a7c59]" : "text-[#8b7e74]"}`}>
                      ...
                      <td className="p-4 text-right">
                        <div className="flex flex-col items-end">
                          {showTierPrice && (
                            <span className="text-[10px] line-through text-[#8b7e74]/40">{formatPrice(pricing.retailPrice)}</span>
                          )}
                          <span
                            className="font-semibold text-[#6b4423]"
                            style={{ fontFamily: "'Playfair Display', 'Source Serif 4', Georgia, serif" }}
                          >
                            {formatPrice(pricing.price)}
                          </span>
                      ...
                      <td className="p-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => handleQtyChange(product.id, -1, moq)}
                            disabled={qty <= moq}
                            className="w-7 h-7 rounded-xl bg-[#faf8f5] border border-[#e8e0d8] text-xs font-medium flex items-center justify-center disabled:opacity-30 transition-colors"
                          >
                            −
                          </button>
                          <span
                          ...
                          <button
                            onClick={() => handleQtyChange(product.id, 1, moq)}
                            className="w-7 h-7 rounded-xl bg-[#faf8f5] border border-[#e8e0d8] text-xs font-medium flex items-center justify-center transition-colors"
                          >
                            +
                          </button>
                        </div>
                      ...
                      <td className="p-4">
                        <button
                          onClick={() => handleBulkAdd(product)}
                          disabled={!hasStock}
                          className="flex items-center justify-center gap-1.5 py-2 px-4 text-xs font-medium rounded-2xl transition-all duration-300 bg-[#6b4423] text-white hover:bg-[#5a3920] disabled:opacity-40 disabled:cursor-not-allowed shadow-[0_2px_8px_rgba(107,68,35,0.15)]"
                          style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}
                        >
